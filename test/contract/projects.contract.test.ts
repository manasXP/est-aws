...
describe('STR-031 T-C2 — admin project CRUD API contract', () => {
  it('POST /v1/projects creates a project conforming to the Admin OpenAPI', async () => {
    ...
    const response = await dispatchRequest('POST', '/v1/projects', { name, description: 'A test project' });
  ...
  it('GET /v1/projects lists the created project', async () => {
    ...
    const createResponse = await dispatchRequest('POST', '/v1/projects', { name });
    const projectId = (createResponse.body as { project_id: string }).project_id;

    const listResponse = await dispatchRequest('GET', '/v1/projects');
  ...
  it('GET /v1/projects/{projectId} and PATCH conform to the Admin OpenAPI', async () => {
    const createResponse = await dispatchRequest('POST', '/v1/projects', { name: `Patchable Project ${randomUUID()}` });
    const projectId = (createResponse.body as { project_id: string }).project_id;

    const getResponse = await dispatchRequest('GET', `/v1/projects/${projectId}`);
    ...
    const patchResponse = await dispatchRequest('PATCH', `/v1/projects/${projectId}`, { description: 'Updated description' });
