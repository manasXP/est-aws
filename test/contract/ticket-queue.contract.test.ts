...
async function createActiveMember(): Promise<string> {
  const response = await dispatchRequest('POST', '/v1/members', { name: `STR-126 Member ${randomUUID()}` });
...
async function adminEmployee(): Promise<string> {
  const response = await dispatchRequest('POST', '/v1/employees', { name: `STR-126 Admin ${randomUUID()}` });
...
describe('STR-126 T-C1 — the admin triage queue contract', () => {
  it('GET /v1/tickets returns a Page of Tickets, oldest first, defaulting to open', async () => {
    ...
    const response = await dispatchRequest('GET', `/v1/tickets?member_id=${memberId}`);
  ...
  it('GET /v1/tickets honors category, status, assignee_id and member_id independently and combined', async () => {
    ...
    const byCategory = await dispatchRequest('GET', `/v1/tickets?member_id=${memberId}&category=finance`);
    ...
    const byStatus = await dispatchRequest('GET', `/v1/tickets?member_id=${memberId}&status=in_progress`);
    expect(items(byStatus.body).map(t => t.ticket_id)).toEqual([maintenance]);

    const byAssignee = await dispatchRequest('GET', `/v1/tickets?assignee_id=${staffId}&status=all`);
    ...
    const noMatch = await dispatchRequest('GET', `/v1/tickets?member_id=${memberId}&category=records&status=all`);
  ...
  it('GET /v1/tickets/{ticketId} conforms to TicketDetail, and 404s an unknown id', async () => {
    ...
    const response = await dispatchRequest('GET', `/v1/tickets/${ticketId}`);
    ...
    const missing = await dispatchRequest('GET', '/v1/tickets/no-such-ticket');
...
describe('STR-126 T-C2 — the mobile own-tickets surface (covers TC-TKT-026)', () => {
  it('GET /me/tickets returns only the caller’s tickets, never another member’s', async () => {
    ...
    const response = await dispatchRequest('GET', '/v1/me/tickets', {}, { 'X-Actor-Member-Id': mine });
  ...
  it('GET /me/tickets honors ?status= and defaults to all statuses', async () => {
    ...
    const all = await dispatchRequest('GET', '/v1/me/tickets', {}, { 'X-Actor-Member-Id': memberId });
  ...
  it('GET /me/tickets/{ticketId} conforms to TicketDetail for the owner and 404s another member', async () => {
    ...
    await dispatchRequest(
      ...
      { body: 'Any update?' },
      { 'X-Actor-Member-Id': owner },
    );

    const op = await contractTest('mobile', '/me/tickets/{ticketId}', 'get');

    const mine = await dispatchRequest('GET', `/v1/me/tickets/${ticketId}`, {}, { 'X-Actor-Member-Id': owner });
    expect(mine.status).toBe(200);
    expect(mine.body).toMatchObject({ ticket_id: ticketId });
    expect(() => op.expectValidResponse(mine.status, mine.body)).not.toThrow();

    const notMine = await dispatchRequest('GET', `/v1/me/tickets/${ticketId}`, {}, { 'X-Actor-Member-Id': stranger });
